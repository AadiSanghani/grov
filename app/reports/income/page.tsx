...
export default function IncomePage() {
  ...
  const categoryIncomeData = useMemo(() => {
    ...
    return Object.entries(byCategory)
      ...
      .sort((a, b) => b.amount - a.amount) as CategoryIncome[]
  }, [incomingTransactions, categories])
  ...
  const summary = useMemo(() => {
    ...
    const categoryCount = categoryIncomeData.length
    const top = categoryIncomeData[0]
    return {
      totalIncome,
      categoryCount,
      topCategory: top
  ...
  }, [incomingTransactions, categoryIncomeData])

  const displayedTransactions = useMemo(() => {
    if (selectedCategory) {
      return incomingTransactions.filter(
        (t) => t.category === selectedCategory
      )
    }
    return incomingTransactions.slice(0, RECENT_TRANSACTIONS_LIMIT)
  }, [incomingTransactions, selectedCategory])

  const selectedCategoryInfo = useMemo(() => {
    if (!selectedCategory) return null
    const info = findCategoryByValue(categories, selectedCategory)
    const data = categoryIncomeData.find((d) => d.value === selectedCategory)
    return info && data
      ? { emoji: info.emoji, label: info.label }
      : { emoji: "📦", label: selectedCategory }
  }, [selectedCategory, categories, categoryIncomeData])
            ...
            {categoryIncomeData.length === 0 ? (
              ...
              <div className="h-[280px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={categoryIncomeData as unknown as Record<string, unknown>[]}
                      dataKey="amount"
                      nameKey="name"
                      cx="50%"
                      cy="50%"
                      ...
                      onClick={(data: unknown) => {
                        const sector = data as { value?: number; payload?: CategoryIncome }
                    ...
                    <RechartsTooltip
                      formatter={(value: number, _name: string, props: unknown) => {
                        const p = (props as { payload: CategoryIncome }).payload
                        return [formatCurrency(value), `${p.emoji} ${p.name}`]
                      }}
                      content={({ active, payload }: { active?: boolean; payload?: readonly { payload: CategoryIncome }[] }) => {
